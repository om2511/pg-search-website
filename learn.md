...
- A successful split deployment can still be completely non-functional if backend CORS is left in local-only mode. Frontend deploy success proves only that the static app built, not that browser-to-API communication is permitted.
